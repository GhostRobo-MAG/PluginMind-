...
interface AuthModalProps {
  isOpen: boolean;
  onClose: () => void;
  redirectTo?: string; // Where to redirect after successful auth
}

export function AuthModal({ isOpen, onClose, redirectTo = "/analyze" }: AuthModalProps) {
  ...
  return (
    <GoogleOAuthProvider clientId={process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID!}>
      <div 
        className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in-0 duration-300"
        onClick={handleBackdropClick}
      >
        <div className="bg-white rounded-lg shadow-2xl w-full max-w-md p-8 space-y-6 animate-in zoom-in-95 duration-300 relative">
          {/* Close Button */}
          <button
            onClick={onClose}
          ...
          <div className="space-y-4">
            ...
            <form onSubmit={handleEmailContinue} className="space-y-4">
              ...
              <Button
                ...
                disabled={!email || emailLoading || isLoading}
              >
                {emailLoading ? (
                  <>
